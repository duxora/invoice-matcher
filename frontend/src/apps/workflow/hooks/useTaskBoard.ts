...
export function useTaskBoard(projectFilter: string, statusFilter: string) {
  ...
  // statusFilter values: '' = active (open+in_progress+backlog), 'open', 'in_progress', 'done', 'all'
  if (statusFilter === '') {
    params.set('status', 'open,in_progress,backlog')
  ...
  return { tasks, projects, tasksError, projectsError }
}
